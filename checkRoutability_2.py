...
def main():
  ...
  occurred = set()
  with open("droplist_data.pickle", "rb") as f:
    drop_data = pickle.load(f)
  ip_list = [x for x in drop_data if x[-3] == '/']
      ...
      if x not in occurred:
        for u_ip in random.sample(list(ipaddress.IPv4Network(unicode(x, "utf-8"))),100):
          ...
          if not tree.check_prefix_is_routable(ip):
            counter += 1
        if counter != 0:
          if date_str not in drop_route:
            drop_route[date_str] = [x]
            occurred.add(x)
            drop_counter += 1
          else:
            drop_route[date_str].append(x)
            occurred.add(x)
            drop_counter += 1

          date_added = datetime.strptime(drop_data[x][0],"%Y-%m-%d").date()
          ...
          if counter != 100:
            weird_counter += 1
  f1 = open( "droproute_2.pickle", "wb" )
  pickle.dump( drop_route, f1)
  f1.close()

  avg_day = diff_time/86400/diff_amount
  print("drop_part2")
  print("drop_counter: " + str(drop_counter))
  print(drop_route)
